...
def test_get_banks_shows_three_expected_banks(api_client, base_url):
...
def test_create_deposit_and_verify_latest_for_banque_misr(api_client, base_url):
    payload = _new_deposit_payload("TEST_BM")
    create = api_client.post(f"{base_url}/api/banks/banque-misr/deposits", json=payload)
    ...
    latest = api_client.get(f"{base_url}/api/banks/banque-misr/deposits/latest")
...
def test_data_isolation_between_banks(api_client, base_url):
    agri_payload = _new_deposit_payload("TEST_AGRI")
    agri_create = api_client.post(f"{base_url}/api/banks/agricultural-bank/deposits", json=agri_payload)
    assert agri_create.status_code == 200
    agri_created_id = agri_create.json()["id"]

    industrial_payload = _new_deposit_payload("TEST_IDB")
    industrial_create = api_client.post(f"{base_url}/api/banks/industrial-development/deposits", json=industrial_payload)
    assert industrial_create.status_code == 200

    misr_deposits = api_client.get(f"{base_url}/api/banks/banque-misr/deposits")
...
def test_current_year_report_has_12_rows_and_summary(api_client, base_url):
    latest = api_client.get(f"{base_url}/api/banks/banque-misr/deposits/latest")
    assert latest.status_code == 200
    deposit = latest.json()
...
def test_previous_year_report_has_12_rows_and_summary(api_client, base_url):
    latest = api_client.get(f"{base_url}/api/banks/banque-misr/deposits/latest")
    assert latest.status_code == 200
    deposit = latest.json()
...
def test_rejects_maturity_before_creation_with_arabic_message(api_client, base_url):
    now = datetime.now(timezone.utc).replace(microsecond=0)
    invalid_payload = {
    ...
    response = api_client.post(f"{base_url}/api/banks/banque-misr/deposits", json=invalid_payload)
