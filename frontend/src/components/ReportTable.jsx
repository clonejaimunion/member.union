...
export const ReportTable = ({ rows, total }) => {
  const totalActiveDays = rows.reduce((sum, row) => sum + Number(row.active_days || 0), 0);
      ...
      <Table data-testid="monthly-interest-table">
        ...
        <TableBody>
          ...
          <TableRow className="bg-amber-50 hover:bg-amber-50" data-testid="report-total-row">
            <TableCell colSpan={2} className="text-lg font-extrabold text-slate-950" data-testid="report-total-label">إجمالي العائد المستحق</TableCell>
            <TableCell className="text-lg font-extrabold text-slate-950" data-testid="report-total-active-days">{formatNumber(totalActiveDays)}</TableCell>
