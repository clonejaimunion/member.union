...
export default function BankReconciliationPage() {
  ...
  const totals = useMemo(() => {
    ...
    const outstanding = outstandingChecks.reduce((sum, item) => sum + Number(sanitizeDecimalInput(item.amount) || 0), 0) + priorYear;
  ...
  const saveReconciliation = async (event) => {
    ...
    try {
      const payload = {
        ...
        prior_year_outstanding_checks: cleanChecks(priorYearChecks),
      };
      ...
      {type === "outstanding" && (
        <div className="mt-5 rounded-lg border border-amber-200 bg-amber-50 p-4 print:hidden" data-testid="prior-year-checks-block">
          <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between" data-testid="prior-year-checks-heading">
            <div>
              <h4 className="text-base font-extrabold text-amber-900" data-testid="prior-year-checks-title">شيكات لم تقدم للصرف عن سنوات سابقة</h4>
              <p className="text-xs font-bold text-amber-700" data-testid="prior-year-checks-subtitle">تُضاف يدويًا لإجمالي شيكات لم تقدم للصرف وحساب المطابقة (بدون قيود محاسبية)</p>
            </div>
      ...
      <div className="space-y-6" data-testid="bank-reconciliation-page">
        ...
        {activeReconciliation && (
          <section className="reconciliation-memo-page space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm print:flex print:min-h-[95vh] print:flex-col print:border-0 print:shadow-none sm:p-8" data-testid="reconciliation-print-report">
            ...
            <ChecksTable title="يضاف: شيكات لم تقدم للصرف" rows={[...(activeReconciliation.outstanding_checks || []), ...(activeReconciliation.prior_year_outstanding_checks || [])]} testId="outstanding-print" total={activeReconciliation.total_outstanding_checks} />
