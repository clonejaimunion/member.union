      ...
      <div className="space-y-6" data-testid="bank-reconciliation-page">
        ...
        {canEditReconciliation && <form onSubmit={saveReconciliation} className="space-y-6 print:hidden" data-testid="reconciliation-form">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm" data-testid="reconciliation-balance-breakdown-section">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-4" data-testid="reconciliation-balance-breakdown-grid">
              ...
              <div className="rounded-lg bg-emerald-50 p-4" data-testid="breakdown-checks-not-presented-card"><p className="text-xs font-bold text-emerald-700">شيكات لم تقدم للصرف</p><p className="mt-1 font-extrabold">{formatEgpText(balanceBreakdown?.checks_not_presented || 0)}</p></div>
              <div className="rounded-lg bg-emerald-100 p-4" data-testid="breakdown-total-receipts-card"><p className="text-xs font-bold text-emerald-800">الإجمالي قبل الخصم</p><p className="mt-1 font-extrabold">{formatEgpText(balanceBreakdown?.gross_total || 0)}</p></div>
              ...
              <div className="rounded-lg bg-red-50 p-4" data-testid="breakdown-checks-under-collection-card"><p className="text-xs font-bold text-red-700">شيكات تحت التحصيل</p><p className="mt-1 font-extrabold">{formatEgpText(balanceBreakdown?.checks_under_collection || 0)}</p></div>
              <div className="rounded-lg bg-red-100 p-4" data-testid="breakdown-total-payments-card"><p className="text-xs font-bold text-red-800">إجمالي الخصومات</p><p className="mt-1 font-extrabold">{formatEgpText(balanceBreakdown?.total_payments || 0)}</p></div>
        ...
        {activeReconciliation && (
          <section className="space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm print:flex print:min-h-[95vh] print:flex-col print:border-0 print:shadow-none sm:p-8" data-testid="reconciliation-print-report">
            ...
            {activeReconciliation.balance_breakdown && <div className="grid grid-cols-2 gap-2 text-sm" data-testid="print-balance-breakdown"><div className="rounded-lg border p-2" data-testid="print-breakdown-opening">الرصيد الافتتاحي: {formatEgpText(activeReconciliation.balance_breakdown.opening_balance)}</div><div className="rounded-lg border p-2" data-testid="print-breakdown-deposits">فوائد الودائع الشهرية: {formatEgpText(activeReconciliation.balance_breakdown.monthly_deposit_interest ?? activeReconciliation.balance_breakdown.deposit_settlements ?? 0)}</div><div className="rounded-lg border p-2" data-testid="print-breakdown-outstanding">شيكات لم تقدم للصرف: {formatEgpText(activeReconciliation.balance_breakdown.checks_not_presented)}</div><div className="rounded-lg border p-2 font-extrabold" data-testid="print-breakdown-total-receipts">الإجمالي قبل الخصم: {formatEgpText(activeReconciliation.balance_breakdown.gross_total)}</div><div className="rounded-lg border p-2" data-testid="print-breakdown-bank-expenses">المصروفات البنكية: {formatEgpText(activeReconciliation.balance_breakdown.bank_expenses)}</div><div className="rounded-lg border p-2" data-testid="print-breakdown-expenses">إجمالي مصروفات البنك: {formatEgpText(activeReconciliation.balance_breakdown.monthly_expenses)}</div><div className="rounded-lg border p-2" data-testid="print-breakdown-collection">شيكات تحت التحصيل: {formatEgpText(activeReconciliation.balance_breakdown.checks_under_collection)}</div><div className="rounded-lg border p-2 font-extrabold" data-testid="print-breakdown-total-payments">إجمالي الخصومات: {formatEgpText(activeReconciliation.balance_breakdown.total_payments)}</div><div className="rounded-lg border p-2 font-extrabold" data-testid="print-breakdown-final">رصيد التسوية: {formatEgpText(activeReconciliation.balance_breakdown.reconciliation_balance)}</div></div>}
