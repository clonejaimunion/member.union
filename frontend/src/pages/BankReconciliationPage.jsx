      ...
      <div className="space-y-6" data-testid="bank-reconciliation-page">
        ...
        {canEditReconciliation && <form onSubmit={saveReconciliation} className="space-y-6 print:hidden" data-testid="reconciliation-form">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm" data-testid="reconciliation-balance-breakdown-section">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-4" data-testid="reconciliation-balance-breakdown-grid">
              ...
              <div className="rounded-lg bg-slate-100 p-4" data-testid="breakdown-book-balance-card"><p className="text-xs font-bold text-slate-700">الرصيد الدفتري</p><p className="mt-1 font-extrabold">{formatEgpText(balanceBreakdown?.book_balance || 0)}</p></div>
        ...
        {activeReconciliation && (
          <section className="space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm print:flex print:min-h-[95vh] print:flex-col print:border-0 print:shadow-none sm:p-8" data-testid="reconciliation-print-report">
            ...
            {activeReconciliation.balance_breakdown && <div className="grid grid-cols-2 gap-2 text-sm" data-testid="print-balance-breakdown"><div className="rounded-lg border p-2" data-testid="print-breakdown-opening">رصيد أول المدة: {formatEgpText(activeReconciliation.balance_breakdown.opening_balance)}</div><div className="rounded-lg border p-2" data-testid="print-breakdown-revenues">الإيرادات الشهرية: {formatEgpText(activeReconciliation.balance_breakdown.monthly_revenues)}</div><div className="rounded-lg border p-2" data-testid="print-breakdown-deposits">ترصيد ودائع: {formatEgpText(activeReconciliation.balance_breakdown.deposit_settlements)}</div><div className="rounded-lg border p-2" data-testid="print-breakdown-expenses">المصروفات: {formatEgpText(activeReconciliation.balance_breakdown.monthly_expenses)}</div><div className="rounded-lg border p-2" data-testid="print-breakdown-bank-expenses">المصروفات البنكية: {formatEgpText(activeReconciliation.balance_breakdown.bank_expenses)}</div><div className="rounded-lg border p-2 font-extrabold" data-testid="print-breakdown-book-balance">الرصيد الدفتري: {formatEgpText(activeReconciliation.balance_breakdown.book_balance)}</div><div className="rounded-lg border p-2" data-testid="print-breakdown-outstanding">يضاف شيكات لم تقدم للصرف: {formatEgpText(activeReconciliation.balance_breakdown.checks_not_presented)}</div><div className="rounded-lg border p-2" data-testid="print-breakdown-collection">يخصم شيكات تحت التحصيل: {formatEgpText(activeReconciliation.balance_breakdown.checks_under_collection)}</div><div className="rounded-lg border p-2 font-extrabold" data-testid="print-breakdown-final">رصيد التسوية: {formatEgpText(activeReconciliation.balance_breakdown.reconciliation_balance)}</div></div>}
