...
const createManualRow = (statement = "", amount = "") => ({ id: `${Date.now()}-${Math.random()}`, statement, amount });
...
export default function BankingExpensesPage() {
    ...
    try {
      ...
      if (isYearly) {
        const grouped = manualResponses.flatMap((response) => response.data.items || []).reduce((acc, item) => {
          ...
          if (!statement || amount <= 0) return acc;
          acc[statement] = (acc[statement] || 0) + amount;
          return acc;
        }, {});
        setReportRows(Object.entries(grouped).map(([statement, amount]) => ({ id: statement, statement, amount })));
        setManualRows([createManualRow()]);
      } else {
        const items = manualResponses[0]?.data?.items || [];
        const rows = items.length ? items.map((item) => createManualRow(item.statement, String(item.amount || ""))) : [createManualRow()];
        setManualRows(rows);
        setReportRows(items.map((item, index) => ({ id: `${index}-${item.statement}`, statement: item.statement, amount: Number(item.amount || 0) })));
      }
    } catch (error) {
  ...
  const updateManualRow = (rowId, field, value) => {
    setManualRows((current) => current.map((row) => (row.id === rowId ? { ...row, [field]: field === "amount" ? sanitizeDecimalInput(value) : value } : row)));
  };
  ...
  const saveManual = async () => {
    ...
    const items = manualRows
      .map((row) => ({ statement: row.statement.trim(), amount: Number(sanitizeDecimalInput(row.amount) || 0) }))
      .filter((row) => row.statement && row.amount > 0);
  ...
  const visibleReportRows = useMemo(() => reportRows.filter((row) => String(row.statement || "").trim() && Number(row.amount || 0) > 0), [reportRows]);
  const totalBankExpenses = useMemo(() => visibleReportRows.reduce((sum, row) => sum + Number(row.amount || 0), 0), [visibleReportRows]);
      ...
      <section className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8" data-testid="banking-expenses-content">
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm print:hidden sm:p-8" data-testid="banking-expenses-filters-section">
          <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between" data-testid="banking-expenses-filters-heading">
            <div><h2 className="text-3xl font-extrabold" data-testid="banking-expenses-filters-title">اختيارات التقرير</h2><p className="mt-1 text-sm font-bold text-slate-500" data-testid="banking-expenses-rules-source">يتم إدخال البيان والمبلغ يدوياً بدون تعريفة أو حساب تلقائي.</p></div>
        ...
        {canManage && !isYearly && (
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm print:hidden sm:p-8" data-testid="banking-expenses-manual-section">
            <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between" data-testid="banking-expenses-manual-heading"><h2 className="text-2xl font-extrabold" data-testid="banking-expenses-manual-title">إدخال البيان والمبلغ</h2><div className="flex flex-wrap gap-2" data-testid="banking-expenses-manual-actions"><Button type="button" onClick={addManualRow} variant="outline" className="h-11 rounded-lg bg-white" data-testid="add-banking-expense-manual-row-button"><Plus className="h-4 w-4" /> إضافة بيان</Button><Button onClick={saveManual} disabled={savingManual} className="h-11 rounded-lg bg-slate-950 text-white" data-testid="save-banking-expenses-manual-button"><Save className="h-4 w-4" /> {savingManual ? "جاري الحفظ..." : "حفظ"}</Button></div></div>
            <div className="space-y-3" data-testid="banking-expenses-manual-grid">
              {manualRows.map((row, index) => <div key={row.id} className="grid grid-cols-1 gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 md:grid-cols-[1fr_220px_auto] md:items-end" data-testid={`banking-expenses-manual-row-${index}`}><div data-testid={`banking-expenses-manual-statement-wrapper-${index}`}><Label data-testid={`banking-expenses-manual-statement-label-${index}`}>البيان</Label><Input value={row.statement} onChange={(event) => updateManualRow(row.id, "statement", event.target.value)} className="mt-2 h-12 rounded-lg bg-white text-right" data-testid={`banking-expenses-manual-statement-input-${index}`} /></div><div data-testid={`banking-expenses-manual-amount-wrapper-${index}`}><Label data-testid={`banking-expenses-manual-amount-label-${index}`}>المبلغ</Label><Input inputMode="decimal" value={row.amount} onChange={(event) => updateManualRow(row.id, "amount", event.target.value)} className="mt-2 h-12 rounded-lg bg-white text-right" data-testid={`banking-expenses-manual-amount-input-${index}`} /></div><Button type="button" onClick={() => removeManualRow(row.id)} variant="outline" className="h-12 rounded-lg border-red-200 bg-red-50 text-red-700 hover:bg-red-100" data-testid={`remove-banking-expense-manual-row-button-${index}`}><Trash2 className="h-4 w-4" /> حذف</Button></div>)}
            </div>
          </section>
        )}

        {canManage && isYearly && <section className="rounded-xl border border-slate-200 bg-white p-5 text-sm font-bold text-slate-600 shadow-sm print:hidden sm:p-8" data-testid="banking-expenses-yearly-manual-note">العرض السنوي يجمع البنود اليدوية المحفوظة في الشهور. للتعديل اختر عرض شهري ثم أدخل البيان والمبلغ.</section>}
        ...
        <section className="space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm print:border-0 print:shadow-none sm:p-8" data-testid="banking-expenses-report-section">
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200" data-testid="banking-expenses-table-wrapper">
            <Table data-testid="banking-expenses-table">
              <TableHeader className="bg-slate-950"><TableRow className="hover:bg-slate-950" data-testid="banking-expenses-table-header-row"><TableHead className="text-right font-extrabold text-white" data-testid="banking-expenses-header-statement">البيان</TableHead><TableHead className="text-right font-extrabold text-white" data-testid="banking-expenses-header-bank-expense">المبلغ</TableHead></TableRow></TableHeader>
              <TableBody>{visibleReportRows.length === 0 && <TableRow data-testid="banking-expenses-empty-report-row"><TableCell colSpan={2} className="py-8 text-center font-extrabold text-slate-500" data-testid="banking-expenses-empty-report-message">لا توجد بنود مصروفات بنكية لهذه الفترة</TableCell></TableRow>}{visibleReportRows.map((row, index) => <TableRow key={`${row.statement}-${index}`} data-testid={`banking-expenses-row-${index}`}><TableCell className="font-extrabold" data-testid={`banking-expenses-row-${index}-statement`}>{row.statement}</TableCell><TableCell className="font-extrabold" data-testid={`banking-expenses-row-${index}-expense`}>{formatCurrency(row.amount)}</TableCell></TableRow>)}<TableRow className="bg-emerald-50 font-extrabold hover:bg-emerald-50" data-testid="banking-expenses-grand-total-row"><TableCell data-testid="banking-expenses-grand-total-label">الإجمالي</TableCell><TableCell data-testid="banking-expenses-grand-total-value">{formatCurrency(totalBankExpenses)}</TableCell></TableRow></TableBody>
            </Table>
          </div>
        </section>
      </section>
