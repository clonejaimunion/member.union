...
import { ArrowRight, FileCheck2, Home, LogOut, Plus, RotateCcw, Save, Wand2 } from "lucide-react";
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950" data-testid="electronic-invoice-page">
      ...
      <section className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8" data-testid="electronic-invoice-content">
        ...
        <section className="space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm print:border-0 print:shadow-none sm:p-8" data-testid="einvoices-report-section"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-sm font-extrabold text-emerald-700">الفواتير المولدة من الإيرادات المحصلة</p><h2 className="text-3xl font-extrabold" data-testid="einvoices-report-title">الفاتورة الإلكترونية</h2></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><div className="rounded-xl bg-slate-950 p-4 text-white"><p className="text-xs font-bold text-slate-300">الصافي</p><p className="text-xl font-extrabold" data-testid="einvoices-net-total">{formatCurrency(totals.net)}</p></div><div className="rounded-xl bg-amber-50 p-4 text-amber-900"><p className="text-xs font-bold text-amber-700">الضريبة</p><p className="text-xl font-extrabold" data-testid="einvoices-tax-total">{formatCurrency(totals.tax)}</p></div><div className="rounded-xl bg-emerald-50 p-4 text-emerald-900"><p className="text-xs font-bold text-emerald-700">الإجمالي</p><p className="text-xl font-extrabold" data-testid="einvoices-total">{formatCurrency(totals.total)}</p></div></div></div><div className="overflow-hidden rounded-xl border border-slate-200"><Table data-testid="einvoices-table"><TableHeader className="bg-slate-950"><TableRow className="hover:bg-slate-950"><TableHead className="text-right text-white">رقم الفاتورة</TableHead><TableHead className="text-right text-white">التاريخ</TableHead><TableHead className="text-right text-white">العميل/الجهة</TableHead><TableHead className="text-right text-white">كود الخدمة</TableHead><TableHead className="text-right text-white">البيان</TableHead><TableHead className="text-right text-white">الصافي</TableHead><TableHead className="text-right text-white">الضريبة</TableHead><TableHead className="text-right text-white">الإجمالي</TableHead><TableHead className="text-right text-white">الحالة</TableHead></TableRow></TableHeader><TableBody>{invoices.length === 0 && <TableRow data-testid="einvoices-empty-row"><TableCell colSpan={9} className="py-8 text-center font-extrabold text-slate-500">لا توجد فواتير إلكترونية في الفترة المختارة</TableCell></TableRow>}{invoices.map((item) => <TableRow key={item.id} data-testid={`einvoice-row-${item.id}`}><TableCell className="font-extrabold">{item.invoice_number}</TableCell><TableCell>{item.issue_date}</TableCell><TableCell>{item.customer_name}</TableCell><TableCell>{item.service_code}</TableCell><TableCell className="max-w-xs break-words">{item.description}</TableCell><TableCell>{formatCurrency(item.net_amount)}</TableCell><TableCell>{formatCurrency(item.tax_amount)}</TableCell><TableCell className="font-extrabold">{formatCurrency(item.total_amount)}</TableCell><TableCell><Badge className="bg-slate-50 text-slate-700 hover:bg-slate-50">{statusLabels[item.status] || item.status}</Badge>{item.validation_notes?.length > 0 && <p className="mt-1 text-xs font-bold text-amber-700">{item.validation_notes.join("، ")}</p>}</TableCell></TableRow>)}</TableBody></Table></div></section>
      </section>
