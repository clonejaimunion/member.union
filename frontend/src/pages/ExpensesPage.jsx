...
import { ArrowRight, Eye, Home, LogOut, MinusCircle, Pencil, Plus, Printer, RotateCcw, Save, Search, SendToBack, Trash2, X } from "lucide-react";
...
export default function ExpensesPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950" data-testid="expenses-page">
      ...
      {selectedExpense && <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-950/60 p-4" data-testid="expense-invoice-modal-overlay"><section className="w-full max-w-6xl rounded-xl bg-white p-4 shadow-2xl" role="dialog" aria-modal="true" data-testid="expense-invoice-modal"><div className="mb-4 flex flex-wrap items-center justify-between gap-3 print:hidden" data-testid="expense-invoice-actions"><h3 className="text-2xl font-extrabold text-slate-950" data-testid="expense-invoice-modal-title">إذن صرف تفصيلي</h3><div className="flex flex-wrap gap-2" data-testid="expense-invoice-action-buttons"><Button type="button" onClick={() => window.print()} className="h-11 rounded-lg bg-slate-950 text-white" data-testid="print-expense-invoice-button"><Printer className="h-4 w-4" /> طباعة PDF</Button><Button type="button" onClick={() => setSelectedExpense(null)} variant="outline" className="h-11 rounded-lg bg-white" data-testid="close-expense-invoice-button"><X className="h-4 w-4" /> إغلاق</Button></div></div><ExpenseVoucher item={selectedExpense} /></section></div>}
