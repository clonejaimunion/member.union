...
import { ArrowRight, CheckCircle2, Eye, Home, LogOut, MinusCircle, Pencil, Plus, RotateCcw, Save, Search, SendToBack, Trash2, X, XCircle } from "lucide-react";
...
export default function ExpensesPage() {
  ...
  const dynamicOrganizationLabels = {
  ...
  const bankExpenseTotals = useMemo(() => {
    const totalsByBank = expenses.reduce((acc, item) => {
      const bankKey = item.bank_id || "unknown-bank";
      if (!acc[bankKey]) {
        acc[bankKey] = {
          bank_id: bankKey,
          bank_name: item.bank_name || banks.find((bank) => bank.id === bankKey)?.name || "بنك غير محدد",
          count: 0,
          gross: 0,
          deductions: 0,
          net: 0,
      ...
      acc[bankKey].count += 1;
      acc[bankKey].gross += Number(item.gross_amount || 0);
      acc[bankKey].deductions += Number(item.total_deductions || 0);
      acc[bankKey].net += Number(item.net_amount || 0);
      return acc;
    }, {});
    return Object.values(totalsByBank).sort((a, b) => a.bank_name.localeCompare(b.bank_name, "ar"));
  }, [banks, expenses]);
  ...
  const editExpense = (item) => {
    ...
    setForm({
      ...
      bank_payment_status: item.bank_payment_status || "not_presented",
    });
  ...
  const updateExpenseBankingStatus = async (item, status) => {
    try {
  ...
  const renderExpenseStatusActions = (item) => {
    ...
    return (
      <div className="grid grid-cols-1 gap-2" data-testid={`expense-row-${item.id}-status-actions`}>
        ...
        <button type="button" onClick={() => updateExpenseBankingStatus(item, "not_presented")} className={`inline-flex h-9 items-center justify-center gap-2 rounded-lg border px-3 text-xs font-extrabold ${status === "not_presented" ? "border-slate-900 bg-slate-950 text-white" : "border-slate-200 bg-slate-50 text-slate-700"}`} data-testid={`mark-expense-under-collection-button-${item.id}`}><XCircle className="h-4 w-4" /> {expenseCollectionStatusLabels.not_presented}</button>
      </div>
    );
  };
