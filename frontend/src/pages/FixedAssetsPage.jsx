...
export default function FixedAssetsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [categoriesResponse, banksResponse, assetsResponse, depreciationResponse] = await Promise.all([
        ...
        api.get(`/fixed-assets/depreciations?year=${runPeriod.year}&month=${runPeriod.month}`),
      ]);
  ...
  }, [runPeriod.year, runPeriod.month]);

  useEffect(() => { loadData(); }, [loadData]);

  const activeCategory = useMemo(() => categories.find((item) => item.code === form.category_code), [categories, form.category_code]);
  const totals = useMemo(() => assets.reduce((acc, asset) => ({ cost: acc.cost + Number(asset.purchase_cost || 0), depreciation: acc.depreciation + Number(asset.accumulated_depreciation || 0), net: acc.net + Number(asset.net_book_value || 0) }), { cost: 0, depreciation: 0, net: 0 }), [assets]);
  ...
  const updateCategory = (categoryCode) => {
    ...
    setForm((current) => ({ ...current, category_code: categoryCode, asset_name: category?.items?.[0] || "" }));
  };
  ...
  const createAsset = async (event) => {
    ...
    setSaving(true);
    try {
      await api.post("/fixed-assets", { ...form, purchase_cost: Number(form.purchase_cost || 0), invoice_number: form.invoice_number || null, notes: form.notes || null, is_active: true });
      toast.success("تم تسجيل الأصل وإنشاء قيد الشراء تلقائياً");
      setForm((current) => ({ ...initialForm, category_code: current.category_code, asset_name: activeCategory?.items?.[0] || "", bank_id: current.bank_id, purchase_date: todayIso }));
      await loadData();
    } catch (error) {
  ...
  const runDepreciation = async () => {
    setSaving(true);
    try {
      const response = await api.post("/fixed-assets/depreciation/run", { year: Number(runPeriod.year), month: Number(runPeriod.month) });
      setDepreciations(response.data);
      toast.success(`تم إثبات إهلاك الشهر تلقائياً لعدد ${response.data.length} أصل`);
      await loadData();
    } catch (error) {
      ...
      <section className="mx-auto grid max-w-7xl grid-cols-1 gap-6 px-4 py-8 sm:px-6 xl:grid-cols-[0.8fr_1.2fr] lg:px-8" data-testid="fixed-assets-content">
        {canManage && <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm print:hidden" data-testid="fixed-asset-create-section">
          ...
          <form onSubmit={createAsset} className="space-y-4" data-testid="fixed-asset-create-form">
            ...
            <div className="rounded-lg bg-emerald-50 p-3 text-sm font-extrabold text-emerald-800" data-testid="fixed-asset-auto-calculation-note">معدل الإهلاك السنوي المختار: {activeCategory?.annual_depreciation_rate || 0}% — سيتم إنشاء قيد شراء الأصل تلقائياً، وإثبات الإهلاك الشهري من الزر المخصص.</div>
