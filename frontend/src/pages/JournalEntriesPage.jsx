...
const sourceLabels = { manual: "يدوي", revenue: "إيراد", expense: "مصروف", banking_expense: "مصروف بنكي", deposit: "ربط وديعة", deposit_interest: "عائد وديعة", reconciliation: "تسوية بنكية", fixed_asset: "أصل ثابت", asset_depreciation: "إهلاك أصل", custody_advance: "عهدة/سلفة", custody_advance_settlement: "تسوية عهدة/سلفة", opening_balance: "رصيد افتتاحي", membership_batch_payment: "إذن عضوية جماعي", inventory: "مخزون", misc_creditor: "دائن متنوع" };
const debitLine = (entry) => entry.lines?.find((line) => Number(line.debit || 0) > 0) || {};
const creditLine = (entry) => entry.lines?.find((line) => Number(line.credit || 0) > 0) || {};
...
export default function JournalEntriesPage() {
  ...
  const [filters, setFilters] = useState({ from_date: "", to_date: "", source_type: "", entry_category: "all", entry_item: "all" });
  ...
  const loadEntries = useCallback(async () => {
    ...
    try {
      ...
      if (filters.entry_category && filters.entry_category !== "all") params.set("entry_category", filters.entry_category);
      if (filters.entry_item && filters.entry_item !== "all") params.set("entry_item", filters.entry_item);
  ...
  }, [filters.from_date, filters.to_date, filters.source_type, filters.entry_category, filters.entry_item]);

  useEffect(() => { loadEntries(); }, [loadEntries]);
  useEffect(() => { api.get("/journal-entry-classifications").then((response) => setClassifications(response.data.categories || [])).catch(() => setClassifications([])); }, []);
  const selectedClassification = classifications.find((item) => item.key === filters.entry_category) || classifications[0] || { items: [{ key: "all", label: "الكل" }] };
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950" data-testid="journal-entries-page">
      ...
      <section className="mx-auto grid max-w-7xl grid-cols-1 gap-6 px-4 py-8 sm:px-6 xl:grid-cols-[0.8fr_1.2fr] lg:px-8" data-testid="journal-entries-content">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm" data-testid="journal-entries-list-section">
          <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><h2 className="text-2xl font-extrabold" data-testid="journal-entries-list-title">دفتر القيود اليومية</h2><p className="text-sm font-bold text-slate-500">عرض وبحث فقط للقيود المرحلة؛ لا تعديل من هذه الشاشة.</p></div><div className="grid grid-cols-1 gap-2 md:grid-cols-6"><div><Label data-testid="journal-filter-from-label">من تاريخ</Label><Input type="date" value={filters.from_date} onChange={(e) => setFilters((c) => ({ ...c, from_date: e.target.value }))} data-testid="journal-filter-from-date" /></div><div><Label data-testid="journal-filter-to-label">إلى تاريخ</Label><Input type="date" value={filters.to_date} onChange={(e) => setFilters((c) => ({ ...c, to_date: e.target.value }))} data-testid="journal-filter-to-date" /></div><div><Label data-testid="journal-filter-entry-category-label">صنف القيد</Label><select value={filters.entry_category} onChange={(e) => setFilters((c) => ({ ...c, entry_category: e.target.value, entry_item: "all", source_type: "" }))} className="h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm font-bold" data-testid="journal-filter-entry-category"><option value="all">كل أصناف القيود</option>{classifications.filter((item) => item.key !== "all").map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</select></div><div><Label data-testid="journal-filter-entry-item-label">نوع القيد</Label><select value={filters.entry_item} onChange={(e) => setFilters((c) => ({ ...c, entry_item: e.target.value }))} className="h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm font-bold" data-testid="journal-filter-entry-item">{selectedClassification.items.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</select></div><div><Label data-testid="journal-filter-source-type-label">المصدر</Label><select value={filters.source_type} onChange={(e) => setFilters((c) => ({ ...c, source_type: e.target.value, entry_category: "all", entry_item: "all" }))} className="h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm font-bold" data-testid="journal-filter-source-type"><option value="">كل المصادر</option>{Object.entries(sourceLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div><Button onClick={loadEntries} variant="outline" className="mt-5 h-10 bg-white" data-testid="journal-filter-apply-button">تطبيق</Button></div></div>
          <div className="overflow-x-auto rounded-xl border border-slate-200" data-testid="journal-entries-table-wrapper"><Table data-testid="journal-entries-table"><TableHeader className="bg-slate-950"><TableRow className="hover:bg-slate-950"><TableHead className="text-right text-white">التاريخ</TableHead><TableHead className="text-right text-white">رقم القيد</TableHead><TableHead className="text-right text-white">البيان</TableHead><TableHead className="text-right text-white">الحساب المدين</TableHead><TableHead className="text-right text-white">الحساب الدائن</TableHead><TableHead className="text-right text-white">المبلغ</TableHead><TableHead className="text-right text-white">حالة الترحيل</TableHead></TableRow></TableHeader><TableBody>{loading && <TableRow><TableCell colSpan={7} className="py-8 text-center font-bold">جاري التحميل...</TableCell></TableRow>}{!loading && entries.length === 0 && <TableRow data-testid="journal-entries-empty-row"><TableCell colSpan={7} className="py-8 text-center font-bold text-slate-500">لا توجد قيود يومية</TableCell></TableRow>}{entries.map((entry) => <TableRow key={entry.id} data-testid={`journal-entry-row-${entry.id}`}><TableCell>{formatDate(entry.entry_date)}</TableCell><TableCell className="font-extrabold" data-testid={`journal-entry-${entry.id}-number`}>{entry.entry_number}</TableCell><TableCell><p className="font-extrabold">{entry.description}</p><p className="mt-1 text-xs text-slate-500">{sourceLabels[entry.source_type] || entry.source_type}</p></TableCell><TableCell>{debitLine(entry).account_name || "—"}</TableCell><TableCell>{creditLine(entry).account_name || "—"}</TableCell><TableCell className="font-extrabold">{formatCurrency(entry.total_debit)}</TableCell><TableCell><Badge className="bg-emerald-50 text-emerald-700 hover:bg-emerald-50">مرحل</Badge></TableCell></TableRow>)}</TableBody></Table></div>
        </section>
      </section>
