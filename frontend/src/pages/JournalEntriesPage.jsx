    ...
    <main className="min-h-screen bg-slate-50 text-slate-950" data-testid="journal-entries-page">
      ...
      <section className="mx-auto grid max-w-7xl grid-cols-1 gap-6 px-4 py-8 sm:px-6 xl:grid-cols-[0.8fr_1.2fr] lg:px-8" data-testid="journal-entries-content">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm" data-testid="journal-entries-list-section">
          <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><h2 className="text-2xl font-extrabold" data-testid="journal-entries-list-title">دفتر القيود اليومية</h2><p className="text-sm font-bold text-slate-500">عرض وبحث فقط للقيود المرحلة؛ لا تعديل من هذه الشاشة.</p></div><div className="grid grid-cols-1 gap-2 md:grid-cols-6"><Input type="date" value={filters.from_date} onChange={(e) => setFilters((c) => ({ ...c, from_date: e.target.value }))} data-testid="journal-filter-from-date" /><Input type="date" value={filters.to_date} onChange={(e) => setFilters((c) => ({ ...c, to_date: e.target.value }))} data-testid="journal-filter-to-date" /><select value={filters.entry_category} onChange={(e) => setFilters((c) => ({ ...c, entry_category: e.target.value, entry_item: "all", source_type: "" }))} className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm font-bold" data-testid="journal-filter-entry-category"><option value="all">كل أصناف القيود</option>{classifications.filter((item) => item.key !== "all").map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</select><select value={filters.entry_item} onChange={(e) => setFilters((c) => ({ ...c, entry_item: e.target.value }))} className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm font-bold" data-testid="journal-filter-entry-item">{selectedClassification.items.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</select><select value={filters.source_type} onChange={(e) => setFilters((c) => ({ ...c, source_type: e.target.value, entry_category: "all", entry_item: "all" }))} className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm font-bold" data-testid="journal-filter-source-type"><option value="">كل المصادر</option>{Object.entries(sourceLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select><Button onClick={loadEntries} variant="outline" className="h-10 bg-white" data-testid="journal-filter-apply-button">تطبيق</Button></div></div>
