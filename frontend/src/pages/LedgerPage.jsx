    ...
    <main className="min-h-screen bg-slate-50 text-slate-950" data-testid="ledger-page">
      ...
      <section className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8" data-testid="ledger-content">
        ...
        <section className="space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm print:border-0 print:shadow-none sm:p-8" data-testid="ledger-report-section">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between" data-testid="ledger-report-heading"><div><p className="text-sm font-extrabold text-emerald-700" data-testid="ledger-report-bank">{isAllAccounts ? "ALL" : report?.account?.code}</p><h2 className="text-3xl font-extrabold" data-testid="ledger-report-title">دفتر الأستاذ - {isAllAccounts ? "عرض الكل" : report?.account?.name || selectedAccount?.name}</h2><p className="mt-1 text-sm font-bold text-slate-500" data-testid="ledger-report-period">الحركات المالية داخل الفترة فقط من {filters.from_date} إلى {filters.to_date}</p></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-3" data-testid="ledger-kpi-grid"><div className="rounded-xl bg-emerald-50 p-4 text-emerald-900" data-testid="ledger-debit-card"><p className="text-xs font-bold text-emerald-700">إجمالي مدين</p><p className="text-xl font-extrabold" data-testid="ledger-debit-total">{formatCurrency(totals.debit)}</p></div><div className="rounded-xl bg-red-50 p-4 text-red-900" data-testid="ledger-credit-card"><p className="text-xs font-bold text-red-700">إجمالي دائن</p><p className="text-xl font-extrabold" data-testid="ledger-credit-total">{formatCurrency(totals.credit)}</p></div><div className="rounded-xl bg-slate-950 p-4 text-white" data-testid="ledger-balance-card"><p className="text-xs font-bold text-slate-300">{isAllAccounts ? "صافي الفترة" : "الرصيد"}</p><p className="text-xl font-extrabold" data-testid="ledger-balance-total">{formatCurrency(totals.balance)}</p></div></div></div>
