...
export default function LedgerPage() {
  ...
  const [filters, setFilters] = useState({ account_id: "", from_date: startOfYear(), to_date: today() });
  const [report, setReport] = useState(null);
  const [loading, setLoading] = useState(true);

  const selectedAccount = useMemo(() => accounts.find((item) => item.id === filters.account_id) || accounts[0], [accounts, filters.account_id]);
  ...
  const loadLedger = useCallback(async () => {
    ...
    try {
      ...
      const accountId = filters.account_id || postableAccounts[0]?.id;
      setAccounts(postableAccounts);
      if (!accountId) {
        setReport(null);
        return;
      }
      if (!filters.account_id) setFilters((current) => ({ ...current, account_id: accountId }));
  ...
  const totals = { debit: report?.total_debit || 0, credit: report?.total_credit || 0, balance: report?.closing_balance || 0 };
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950" data-testid="ledger-page">
      ...
      <section className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8" data-testid="ledger-content">
        ...
        <section className="space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm print:border-0 print:shadow-none sm:p-8" data-testid="ledger-report-section">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between" data-testid="ledger-report-heading"><div><p className="text-sm font-extrabold text-emerald-700" data-testid="ledger-report-bank">{report?.account?.code}</p><h2 className="text-3xl font-extrabold" data-testid="ledger-report-title">دفتر الأستاذ - {report?.account?.name || selectedAccount?.name}</h2><p className="mt-1 text-sm font-bold text-slate-500" data-testid="ledger-report-period">من {filters.from_date} إلى {filters.to_date}</p></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-3" data-testid="ledger-kpi-grid"><div className="rounded-xl bg-emerald-50 p-4 text-emerald-900" data-testid="ledger-debit-card"><p className="text-xs font-bold text-emerald-700">إجمالي مدين</p><p className="text-xl font-extrabold" data-testid="ledger-debit-total">{formatCurrency(totals.debit)}</p></div><div className="rounded-xl bg-red-50 p-4 text-red-900" data-testid="ledger-credit-card"><p className="text-xs font-bold text-red-700">إجمالي دائن</p><p className="text-xl font-extrabold" data-testid="ledger-credit-total">{formatCurrency(totals.credit)}</p></div><div className="rounded-xl bg-slate-950 p-4 text-white" data-testid="ledger-balance-card"><p className="text-xs font-bold text-slate-300">الرصيد</p><p className="text-xl font-extrabold" data-testid="ledger-balance-total">{formatCurrency(totals.balance)}</p></div></div></div>
          <div className="overflow-hidden rounded-xl border border-slate-200" data-testid="ledger-table-wrapper"><Table data-testid="ledger-table"><TableHeader className="bg-slate-950"><TableRow className="hover:bg-slate-950" data-testid="ledger-table-header-row"><TableHead className="text-right font-extrabold text-white" data-testid="ledger-header-serial">م</TableHead><TableHead className="text-right font-extrabold text-white" data-testid="ledger-header-date">التاريخ</TableHead><TableHead className="text-right font-extrabold text-white" data-testid="ledger-header-source">المصدر</TableHead><TableHead className="text-right font-extrabold text-white" data-testid="ledger-header-reference">المرجع</TableHead><TableHead className="text-right font-extrabold text-white" data-testid="ledger-header-statement">البيان</TableHead><TableHead className="text-right font-extrabold text-white" data-testid="ledger-header-debit">مدين</TableHead><TableHead className="text-right font-extrabold text-white" data-testid="ledger-header-credit">دائن</TableHead><TableHead className="text-right font-extrabold text-white" data-testid="ledger-header-balance">الرصيد</TableHead></TableRow></TableHeader><TableBody>{rows.length === 0 && <TableRow data-testid="ledger-empty-row"><TableCell colSpan={8} className="py-8 text-center font-extrabold text-slate-500" data-testid="ledger-empty-message">لا توجد حركات في الفترة المختارة</TableCell></TableRow>}{rows.map((row) => <TableRow key={`${row.entry_id}-${row.serial}`} data-testid={`ledger-row-${row.serial}`}><TableCell data-testid={`ledger-row-${row.serial}-serial`}>{formatNumber(row.serial)}</TableCell><TableCell data-testid={`ledger-row-${row.serial}-date`}>{row.entry_date}</TableCell><TableCell data-testid={`ledger-row-${row.serial}-source`}>{row.source_type}</TableCell><TableCell data-testid={`ledger-row-${row.serial}-reference`}>{row.reference || "—"}</TableCell><TableCell className="font-bold" data-testid={`ledger-row-${row.serial}-statement`}>{row.description}</TableCell><TableCell data-testid={`ledger-row-${row.serial}-debit`}>{row.debit ? formatCurrency(row.debit) : "—"}</TableCell><TableCell data-testid={`ledger-row-${row.serial}-credit`}>{row.credit ? formatCurrency(row.credit) : "—"}</TableCell><TableCell className="font-extrabold" data-testid={`ledger-row-${row.serial}-balance`}>{formatCurrency(row.balance)}</TableCell></TableRow>)}<TableRow className="bg-slate-50 font-extrabold hover:bg-slate-50" data-testid="ledger-grand-total-row"><TableCell colSpan={5} data-testid="ledger-grand-total-label">الإجمالي</TableCell><TableCell data-testid="ledger-grand-total-debit">{formatCurrency(totals.debit)}</TableCell><TableCell data-testid="ledger-grand-total-credit">{formatCurrency(totals.credit)}</TableCell><TableCell data-testid="ledger-grand-total-balance">{formatCurrency(totals.balance)}</TableCell></TableRow></TableBody></Table></div>
        </section>
      </section>
