...
export default function MembershipPage() {
  ...
  const importMembers = async (event) => {
    ...
    try {
      ...
      payload.append("status", importForm.status || "active");
      if (importForm.status !== "active" && importForm.status_effective_date) payload.append("status_effective_date", importForm.status_effective_date);
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm print:hidden" data-testid="membership-import-section">
            <form onSubmit={importMembers} className="space-y-4" data-testid="membership-import-form">
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2" data-testid="membership-import-location-grid">
                ...
                <div data-testid="membership-import-status-wrapper"><Label data-testid="membership-import-status-label">حالة العضويات المستوردة</Label><select value={importForm.status} onChange={(event) => updateImportForm("status", event.target.value)} className="mt-2 h-11 w-full rounded-md border border-slate-300 bg-slate-50 px-3 font-bold" data-testid="membership-import-status-select">{statusOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
                <div data-testid="membership-import-status-date-wrapper"><Label data-testid="membership-import-status-date-label">تاريخ الحالة للملف</Label><Input type="date" value={importForm.status_effective_date} onChange={(event) => updateImportForm("status_effective_date", event.target.value)} disabled={importForm.status === "active"} className="mt-2 h-11 bg-slate-50" data-testid="membership-import-status-date-input" /></div>
              </div>
